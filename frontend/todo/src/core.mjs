...
export class SemanticQueue{
  ...
  start(board,discard=false){if(this.unresolved&&!discard)throw new Error('请先处理结果未知的保存');clearTimeout(this.timer);this.generation++;this.ack=clone(board);this.board=clone(board);this.latest=null;if(discard)this.unresolved=null;this.state='saved';this.publish(clone(this.board));this.notify(this.state);}
  ...
  async flush(){clearTimeout(this.timer);if(this.retired)return this.ack;if(this.unresolved)throw this.unresolved.error;if(this.running)return this.running;const generation=this.generation;
    this.running=(async()=>{while(this.latest&&generation===this.generation){const nodes=this.latest;this.latest=null;const operations=diffNodes(this.ack.nodes,nodes);if(!operations.length)continue;const attempt={id:this.ack.id,payload:{revision:this.ack.revision,request_id:uuid(),operations,confirm_destructive:true},nodes,error:null};this.state='saving';this.notify(this.state);
      try{const result=await this.perform(attempt.id,attempt.payload);if(generation!==this.generation)return this.ack;this.ack=clone(result.board);this.board={...clone(this.ack),nodes:clone(this.latest||this.ack.nodes)};this.publish(clone(this.board));}
      catch(error){if(generation!==this.generation)return this.ack;attempt.error=error;this.latest=this.latest||nodes;if(isAmbiguousWriteError(error))this.unresolved=attempt;this.state='error';this.notify(this.state,error);throw error;}}
  ...
  async retry(){if(!this.unresolved)return this.flush();const attempt=this.unresolved;this.state='saving';this.notify(this.state);try{const result=await this.perform(attempt.id,attempt.payload);this.ack=clone(result.board);this.unresolved=null;if(this.latest&&sameNodes(this.latest,attempt.nodes))this.latest=null;this.board={...clone(this.ack),nodes:clone(this.latest||this.ack.nodes)};this.publish(clone(this.board));return this.flush();}catch(error){attempt.error=error;this.state='error';this.notify(this.state,error);throw error;}}
  hasPending(){return Boolean(this.latest||this.running||this.unresolved);}
  retire(){clearTimeout(this.timer);this.retired=true;this.generation++;this.latest=null;}
}
