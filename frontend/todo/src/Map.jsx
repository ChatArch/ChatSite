...
export default function MapEditor({board,rootId,layout,disabled,onNodes,onSelect,onView,onUndo,onRedo,apiRef}){
  ...
  useEffect(()=>{
    ...
    try{
      ...
      const original=mindMap.execCommand.bind(mindMap);mindMap.execCommand=(name,...args)=>{if(name==='BACK'){latest.current.onUndo();return;}if(name==='FORWARD'){latest.current.onRedo();return;}return original(name,...args);};
