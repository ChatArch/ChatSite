...
export default function MapEditor({board,rootId,layout,disabled,onNodes,onSelect,onView,onUndo,onRedo,apiRef}){
  ...
  useEffect(()=>{
    ...
    const emitView=(persistCollapse=false)=>{if(!alive||suppress.current)return;try{const data=mindMap.getData(true),state=mindMap.view.getTransformData().state;const visibleCollapsed=flattenWithExpand(data).filter(x=>!x.expand).map(x=>x.id);const collapsed=persistCollapse===true?mergeCollapsed(latest.current.board.nodes,latest.current.rootId,latest.current.board.view?.collapsed,visibleCollapsed):(latest.current.board.view?.collapsed||[]);latest.current.onView({...latest.current.board.view,pan:{x:state.x,y:state.y},zoom:state.scale,positions:positions.current,collapsed});}catch(reason){setError(reason.message);}};
    try{
      ...
      const original=mindMap.execCommand.bind(mindMap);mindMap.execCommand=(name,...args)=>{if(name==='BACK'){latest.current.onUndo();return;}if(name==='FORWARD'){latest.current.onRedo();return;}const result=original(name,...args);if(name==='SET_NODE_EXPAND')emitView(true);return result;};
