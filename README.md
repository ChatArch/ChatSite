...
此开发分支增加任务树画布、Markdown 内容编辑、独立模型对话、持久化、变更回执与撤销。Web 由 ChatSite 提供，任务领域由 ChatTodo 提供；安装此功能需要对应的 ChatTodo 开发构建。使用 `chatsite todo serve` 启动，`chatsite todo check` 验证配置和模型。配置由 ChatEnv 的 `chatsite-todo` provider 管理。旧的 PyPI 占位包不包含此功能。
